...
df_owid_roser = pd.read_csv(join(dir_gdrive, fn_owid_roser))
...
df_owid_ortiz = pd.read_csv(join(dir_gdrive, fn_owid_ortiz))
...
df_wiki = pd.read_csv(join(dir_gdrive, fn_wiki))
...
df_worldometers = pd.read_csv(join(dir_gdrive, fn_worldometers))
...
df_biominers = pd.read_csv(join(dir_gdrive, fn_biominers))
...
# prep
df_1 = df_owid_roser.rename(columns={"Entity2": "Location", "Date": "Date", "Daily change in cumulative total tests": "total_daily.owid_roser"})
df_1 = df_1[["Location","Date","total_daily.owid_roser"]]

df_2 = df_owid_ortiz.rename(columns={"Country or territory": "Location", "Date": "Date", "Total tests": "total_cumul.owid_ortiz"})
...
df_4 = df_worldometers.rename(columns={"Country, Other": "Location", "Date": "Date", "Total Tests": "total_cumul.worldometers"})
df_4 = df_4[["Location","Date","total_cumul.worldometers"]]

df_5 = df_biominers.rename(columns={"country_t11": "Location", "Date": "Date", "total_cumul": "total_cumul.biominers"})
...
df_agg = df_merged.groupby("Location").agg({"Date": [np.min,np.max,len],
...
df_agg.to_csv(join("/content/", fn_analysis_agg))
df_agg.to_csv(join(dir_gdrive, fn_analysis_agg))
...
df_save.to_csv(join(dir_gdrive, fn_agg), index=False)
